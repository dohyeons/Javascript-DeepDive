...
- 함수가 실행되려면 함수 실행 컨텍스트가 실행 컨텍스트 스택에 푸시되어야 한다. 즉, 실행 컨텍스트에 함수 실행 컨텍스트가 푸시되는 것은 바로 함수 실행의 시작을 의미한다.

- JS엔진은 단 하나의 실행 컨텍스트 스택을 가진다. 즉, 동시에 2개 이상의 함수를 실행할 수 없다. 나머지는 모두 대기중인 태스크이다.

- 따라서 시간이 걸리는 태스크를 실행할 경우 블로킹이 발생한다.

- 이처럼 현재 실행 중인 태스크가 종료할 때까지 다음 태스크가 대기하는 방식을 동기 처리라고 한다. 실행 순서를 보장하지만, 다른 태스크가 블로킹되는 단점이 있다.

- 반대로 비동기 처리는 현재 실행중인 태스크가 종료되지 않은 상태여도 다음 태스크를 곧바로 실행한다. 블로킹이 발생하지 않지만, 작업 순서가 보장되지 않는 단점이 있다.

- `setTimeout`, `setInterval` 은 비동기 방식으로 동작한다.

## 42.2 이벤트 루프와 태스크 큐

- 싱글 스레드인 JS가 동시성을 가지게 해주는 것이 바로 이벤트 루프이다.

- 자바스크립트 엔진은 콜 스택과 힙으로 구분할 수 있음
  - 콜 스택: 실행 컨텍스트 스택.
  - 힙: 객체가 저장되는 메모리 공간. 실행 컨텍스트는 여기서 객체를 참조함

JS엔진은 단순히 콜 스택을 통해 요청된 작업을 순차적으로 실행함.
소스 코드의 평가와 실행을 제외한 모든 처리는 브라우저 또는 Node.js가 담당한다.

> 예를 들어, `setTimeout` 의 콜백 함수의 평가와 실행은 JS엔진이 하지만, 타이머 설정과 콜백 함수 등록은 브라우저 혹은 Node.js가 담당한다.

이를 위해 제공되는 것이 테스크 큐와 이벤트 루프이다.

- 테스크 큐: 비동기 함수의 콜백 함수 혹은 이벤트 핸들러가 일시적으로 보관되는 영역

- 이벤트 루프: 콜 스택이 비어있는지, 태스크 큐에 대기중인 함수가 있는지 확인하고, 콜 스택이 비어있고 태스크 큐에 대기중인 함수가 있다면 순차적으로 테스크 큐에 대기 중인 함수를 콜 스택으로 이동시킴

싱글 스레드 방식으로 동작하는 것은 브라우저가 아니라, 브라우저에 내장된 JS엔진이라는 것에 주의한다. 브라우저는 멀티 스레드 방식으로 동작한다. 브라우저는 JS엔진 외에도 렌더링 엔진과 Web API를 제공한다.
