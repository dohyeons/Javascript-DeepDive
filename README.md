...
이처럼 함수가 외부의 상태를 변경하면 상태 변화를 추적하기가 어렵다. 이는 코드의 복잡성을 증가시키고 가독성을 해치는 원인이 된다. 상태 변화를 감지하기 위해서는 함수 내부를 유심히 봐야하기 때문이다.

## 12.7 다양한 함수의 형태
### 12.7.1 즉시 실행 함수
> 단 한번만 실행되며 이를 다시 호출할 수 없다.
```
let test = 12;
(function foo() {
  test = 15;
...
* 만약 함수 리터럴을 평가해 함수 객체를 생성할 수 있다면 다르 방법을 사용해도 된다.
...
* 그러나 무한 반복에 빠져 스택 오버플로우 에러를 발생시킬 수 있다. => 탈출 조건을 반드시 올바르게 명시해야 한다.
